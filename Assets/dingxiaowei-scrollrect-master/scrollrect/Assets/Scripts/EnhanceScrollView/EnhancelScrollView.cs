...
public class EnhancelScrollView : MonoBehaviour
{
	...
	public float dFactor = 0.1f; //每一次移动的间距

	public int MaxShowCount = 5; //最大的展示数量
	...
	public void Init()
	{
		...
		if (moveHorizontalValues == null)
			moveHorizontalValues = new float[scrollViewItems.Count];  //这个换成最大数量或者count
	...
	{
		int centerIndex = scrollViewItems.Count / 2;
        float centreFactor = 0.5f;

        for (int i = 0; i < scrollViewItems.Count; i++)
		{
			float factor = (centreFactor - dFactor * (centerIndex - i));
